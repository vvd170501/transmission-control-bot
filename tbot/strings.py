...
def help(with_ftp):
    ftp_commands = (
        "/ftp - открыть доступ по FTP к Вашим и общим торрентам\n"
        "/noftp - закрыть FTP-доступ\n"
    )
    # improve style?
    return """\
...
/my_torrents - показать список ваших торрентов
/shared_torrents - поакзать список общих торрентов
{}/disk - показать заполненность диска""".format(ftp_commands if with_ftp else '')
...
class Buttons:
    ...
    move_torrent = '🗂 Переместить торрент'
    share_torrent = '👥 Поделиться торрентом'
    unshare_torrent = '👤 Сделать торрент приватным'
    delete_torrent = '❌ Удалить торрент и скачанные файлы'
    back = '↩ Назад'
